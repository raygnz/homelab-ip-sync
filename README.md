...
- **Azure Storage Firewall Sync**: Updates the Azure Storage Account firewall to allow access from the current Cloudflare IP, ensuring secure and dynamic access.
...
- **Service Principal Authentication**: Uses an Azure AD application and service principal for secure, automated access to Azure resources.
...
3. **Azure Authentication**: The function authenticates to Azure using a service principal.
4. **Firewall Update**: If the IP address has changed, the function updates the Azure Storage Account's firewall rules to allow the new IP and remove the old one.
5. **Secret Management**: The Cloudflare API token can be securely retrieved from Azure Key Vault.

## Files and Structure
- `main.tf`: Terraform configuration for all Azure resources, role assignments, and integration with Key Vault and Cloudflare.
...
- `provider.tf`: Provider configuration for Azure and AzureAD.
- `backend.tf`: Remote state configuration for Terraform.
- `function/`: Contains the Python Azure Function code (`__init__.py`) that performs the sync logic.
...
3. **(Optional) Store secrets** in Azure Key Vault if you want to avoid storing them in plain text.
...
- The Function App uses a managed identity for secure access to Azure resources.
